...
context("FunctionalModel.linear")

test_that("Test FunctionalModel.linear", {
  model <- FunctionalModel.linear();
...
test_that("Test .linear.from.two.points", {
  f   <- function(x, par) par[1] + (par[2] * x);

  par <- c(1, 2);
  expect_equal(par, .linear.from.two.points(3, f(3, par), 4, f(4, par)));

  par <- c(-3, 0);
  expect_equal(par, .linear.from.two.points(2, f(2, par), 5, f(5, par)));

  par <- c(0, 5);
  expect_equal(par, .linear.from.two.points(-4, f(-4, par), 6, f(6, par)));

  par <- c(7, -7);
  expect_equal(par, .linear.from.two.points(7, f(7, par), -7, f(-7, par)));
})
